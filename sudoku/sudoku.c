...
void check_duplicate_in_box(char sudoku[N][N], int *x) {
    int i, j, m, n;
    for (i = 0; i < N; i+=3) {
        for (j = 0; j < N; j+=3) {
            int check[10] = {0}; /*0 to 9 for checking numbers in the 3x3 box*/
            for (m = 0; m < 3; m++) {
                for (n = 0; n < 3; n++) {
                    if (sudoku[i+m][j+n] == UNASSIGNED) continue;
                    int num = sudoku[i+m][j+n] - '0'; /* convert char to int*/
                    if (check[num] != 0) {
                        printf("Error\n\n");
                        *x = 1;
                        return;
              ...
              /*allocate memory for the line and copy it into the array*/
                ...
                if(y!=1){
                    ...
                    if (size != 81) {
                        ...
                        if(x!=1){
                                check_duplicate_in_box(sudoku,&x);
                                if(x!=1){
